...
def compute_full_kl(dist1, dist2):
    dist1 = dist1 + 1e-8
    dist2 = dist2 + 1e-8
    return torch.sum(
        dist1 * (torch.log(dist1) - torch.log(dist2)),
        axis=-1,
    )  # KL divergence
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, distribution = agent.get_action_and_value(
                    next_obs,
                )
        ...
        b_distributions = distributions.reshape(
            (-1, envs.single_action_space.n),
        )
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, new_distribution = (
                ...
                if args.kl_direction == "reverse":
                    if args.use_spma:
                        ...
                        pg_loss2 = -mb_advantages * torch.clamp(logratio, -args.clip_coef, args.clip_coef).exp() 
                        pg_loss = torch.max(pg_loss1, pg_loss2).mean()
                    clipfracs += [logratio.abs().gt(args.clip_coef).float().mean().item()]
                    ...
                    if args.use_spma:
                        ...
                        pg_loss1 = -mb_advantages * (-logratio).exp()
                        pg_loss2 = -mb_advantages * torch.clamp(
                            -logratio,
                            -args.clip_coef,
                            args.clip_coef,).exp()
                    ...
                    raise NotImplementedError(
                        f"Unknown kl_direction: {args.kl_direction}",
                    )
        ...
        writer.add_scalar(
            "losses/grad_norm",
            torch.mean(torch.tensor(grad_norms)).item(),
            global_step,
        )
        writer.add_scalar(
            "losses/ratio",
